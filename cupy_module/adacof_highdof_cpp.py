...
kernel_AdaCoF_updateOutput = '''
    extern "C" __global__ void kernel_AdaCoF_updateOutput(
        ...
        float w         = VALUE_4(weight, intSample, c*k*F_SIZE+l, i, j);
        float alpha     = VALUE_4(offset_i, intSample, c*k*F_SIZE+l, i, j);
        float beta      = VALUE_4(offset_j, intSample, c*k*F_SIZE+l, i, j);
        int A           = (int) alpha;
        int B           = (int) beta;
...
kernel_AdaCoF_updateGradWeight = '''
    extern "C" __global__ void kernel_AdaCoF_updateGradWeight(
        ...
        int d = intDepth / F_SIZE / F_SIZE;
        int k = (intDepth - d * F_SIZE * F_SIZE) / F_SIZE;
        int l = intDepth % F_SIZE;

        for (int c = 0; c < 3; c++) 
        {
        float delta     = VALUE_4(gradLoss, intSample, c, i, j);
        float alpha     = VALUE_4(offset_i, intSample, d * F_SIZE * F_SIZE + k*F_SIZE+l, i, j);
        float beta      = VALUE_4(offset_j, intSample, d * F_SIZE * F_SIZE + k*F_SIZE+l, i, j);
        int A           = (int) alpha;
        int B           = (int) beta;
...
kernel_AdaCoF_updateGradAlpha = '''
    extern "C" __global__ void kernel_AdaCoF_updateGradAlpha(
        ...
        {
        float delta     = VALUE_4(gradLoss, intSample, c, i, j);
        float w         = VALUE_4(weight, intSample, d * F_SIZE * F_SIZE + k*F_SIZE+l, i, j);
        float alpha     = VALUE_4(offset_i, intSample, d * F_SIZE * F_SIZE + k*F_SIZE+l, i, j);
        float beta      = VALUE_4(offset_j, intSample, d * F_SIZE * F_SIZE + k*F_SIZE+l, i, j);
        int A           = (int) alpha;
        int B           = (int) beta;
...
class FunctionAdaCoF(torch.autograd.Function):
    ...
    def forward(ctx, input, weight, offset_i, offset_j, dilation):
        ...
        intFilterSize = int(math.sqrt(weight.size(1)/3))
        ...
        if input.is_cuda == True:
            ...
            n = output.nelement() #4*3*32*32=12288
    ...
    def backward(ctx, gradOutput):
        ...
        gradWeight = input.new_zeros(intSample, intFilterSize ** 2*3, intOutputHeight, intOutputWidth) if ctx.needs_input_grad[1] == True else None
        gradOffset_i = input.new_zeros(intSample, intFilterSize ** 2*3, intOutputHeight, intOutputWidth) if ctx.needs_input_grad[2] == True else None
        gradOffset_j = input.new_zeros(intSample, intFilterSize ** 2*3, intOutputHeight, intOutputWidth) if ctx.needs_input_grad[2] == True else None
